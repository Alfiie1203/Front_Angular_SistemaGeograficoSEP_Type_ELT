...
@Component({
  selector: 'app-validate-form-list',
  templateUrl: './validate-form-list.component.html',
  styleUrls: ['./validate-form-list.component.scss'],
  animations: [
...
export class ValidateFormListComponent implements OnInit {
  constructor(
  ...
  title_name = 'assign_validator_for_form';
  textSearch = '';
  filterValue = '';
  ...
  getProforestForms() {
    ...
    const url = `formulario/list/proforestforms/idnames/`;
    this.apiService
      .getResponse('GET', url)
      .then(
        (resp: any) => {
  ...
  list() {
    ...
    let name =
      this.form_search && this.form_search.get('name')
        ...
        : '';
    const url = `formulario/validate/list/formularios/?limit=${this.pagination}&offset=${this.OFFSET}&name=${name}&revision_status=${revision_status}&proforestform_id=${proforestform_id}&revisor_id=${revisor_id}&period=${periodo}`;
    this.apiService
      .getResponse('GET', url)
      .then(
        (resp: any) => {
  ...
  async saveValidator(validator: validatorsI, event: Event, op_en: any) {
    ...
    const url = `formulario/validate/assign/list/formularios/`;
      this.loadEventService.loadTableEvent.emit(true);
  ...
  async saveValidator2(validator: validatorsI, event: Event, op_en: any) {
    ...
    const url = `formulario/validate/assign/formulario/${this.idForm}`;
      this.loadEventService.loadTableEvent.emit(true);
