...
@Component({
  selector: 'app-assign-company-validator',
  templateUrl: './assign-company-validator.component.html',
  styleUrls: ['./assign-company-validator.component.scss'],
  animations: [
...
export class AssignCompanyValidatorComponent implements OnInit {
  ...
  async list() {
    ...
    if (getToken) {
      const _url = `coordinates/view/validation/company/list/?limit=${this.pageSize}&offset=${this.OFFSET}&name=${
        this.name ? this.name : ''
      }&actor_type=${
      ...
      this.apiService.getResponse('GET', _url).then(
        (resp: any) => {
          ...
          if (resp && resp.results) {
           ...
           // console.log("this.listData.results:",this.listData.results);
  ...
  async saveValidator(validator: validatorsI, event: Event, op_en: any) {
    ...
    if(this.assingMassive==0){
      url = `coordinates/asign/companyvalidator/${this.company.id}/`;
      method = 'PATCH'
    }else if(this.assingMassive==1){
      url = `coordinates/asign/list/company/validator/`;
      method = 'POST'
