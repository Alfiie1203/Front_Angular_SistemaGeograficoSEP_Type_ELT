...
export class VerifyCompanyComponent implements OnInit {
  ...
  displayedColumns: string[] = ['name',"validator_user","deadline", 'action', 'more'];
  listPeriod: any[] = [
    {
      id: 0,
      name: 'ANNUAL',
  ...
  async list() {
      ...
      //console.log(this.periodo);
      let url = `coordinates/verification/company/list/?limit=${this.pageSize}&offset=${this.OFFSET}&year=${this.year}&period=${(this.periodo && this.periodo.id)?this.periodo.id:''}`;
      this.apiService.getResponse('GET', url).then(
        (resp: any) => {
          //console.log('resp', resp)
          if (resp && resp.results) {
            this.listData = resp;
            //console.log('resp',  this.listData)
            //console.log("8000 list:",this.listData);
  ...
  async saveValidation() {
    ...
    //confirm save
    const url = `coordinates/verification/company/${this.company.id}/`;
...
/**************** */
  ...
  addMarker(event: google.maps.MapMouseEvent) {
    ...
    //console.log("Map ->gragMarker",event);
