...
@Component({
  selector: 'app-assign-verify',
  templateUrl: './assign-verify.component.html',
  styleUrls: ['./assign-verify.component.scss'],
  animations: [
...
export class AssignVerifyComponent implements OnInit {@ViewChild(MatPaginator, { static: true })
...
async list() {
  ...
  if (getToken) {
    const _url = `coordinates/view/verification/traceability/list/?name=${
      this.name ? this.name : ''
...
async saveValidator(validator: validatorsI, event: Event, op_en: any) {
  ...
  const url = `coordinates/asign/verification/traceability/list/`;
  let getToken: any;
  this.loading = true
  await this.storageService
    .get('dataInfoKey')
  ...
  if (getToken) {
    ...
    this.apiService.getResponse('POST', url, dataSend).then(
      (resp: any) => {
        ...
        if (resp && resp.results) {
          let title = this.translate.instant(resp.results);
