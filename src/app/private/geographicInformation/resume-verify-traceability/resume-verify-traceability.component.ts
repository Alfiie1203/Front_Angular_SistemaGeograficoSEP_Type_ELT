...
@Component({
  selector: 'app-resume-verify-traceability',
  templateUrl: './resume-verify-traceability.component.html',
  styleUrls: ['./resume-verify-traceability.component.scss']
})
export class ResumeVerifyTraceabilityComponent implements OnInit {
  ...
  async list() {
    ...
    if (getToken) {
      ...
      let url = `coordinates/traceability/verifyresume/company/?limit=${this.pageSize}&offset=${this.OFFSET}&year=${this.year}&period=${(this.periodo && this.periodo.id)?this.periodo.id:''}&company_name=${this.textSearch?this.textSearch:''}`;
  ...
  validateSend(id:any){
    this.router.navigate(['/geographic-information/verify-traceability/'], {queryParams: {idCompany: id}})
  }

  sendEmail(id:any){
    this.apiService.getResponse('GET', `coordinates/traceability/verifyresume/company/sendmail/?reported_company_id=${id}`)
    .then(()=>{
