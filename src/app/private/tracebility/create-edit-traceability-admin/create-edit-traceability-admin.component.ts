...
@Component({
  selector: 'app-create-edit-traceability-admin',
  templateUrl: './create-edit-traceability-admin.component.html',
  styleUrls: ['./create-edit-traceability-admin.component.scss'],
})
export class CreateEditTraceabilityAdminComponent implements OnInit {
  ...
  createForm(data: any = null) {
    ...
    this.form = this.formBuilder.group({
      id_empresa_reporta: [(this.ID?this.ID:''), [Validators.required]],
      nit_reporta: [data ? data.reported_company_nit : '', [Validators.required]],
      name_report: [data ? data.reported_company : '', [Validators.required]],
      nit: [data ? data.supplier_tax_number : '', [Validators.required]],
      name: [data ? data.supplier_name : '', [Validators.required]],
      listbusiness_group: [ data && data.company_group ? data.company_group : ''],
      listCommodity: [data && data.commodity ? data.commodity : '', [Validators.required]],
      ...
      certification: [data && data.certification ? data.certification : '', [Validators.required]],
    });
    this.formCreated = true;
    // console.log("formCreated",this.formCreated);
    // this.form = this.formBuilder.group({
    //   company_group: [''],
    //   listbusiness_group: [''],
    ...
    this.form
      ...
      .subscribe((response) => {
        if(this.setFilter){
          ...
          .getResponse('GET', `company/company/search/detail/?name=${response}`)
          .then((resp: any) => {
            let name_report = document.getElementById("name_report");
            ...
            if (resp && resp.results && resp.results instanceof Array) {
              ...
              this.setDatalistReportForm(name_report)
              /* let find = this.empresasListReport.find(
                (x) => x?.name === response
              );
              this.setFormResultReport(find); */
  ...
  createFormPlantilla() {
    this.formPlantilla = this.formBuilder.group({
      id_empresa_plantilla: ['', [Validators.required]],
      nit_plantilla: ['', [Validators.required]],
      name_plantilla: ['', [Validators.required]],
    ...
    this.formPlantilla
      ...
      .subscribe((response) => {
        this.apiService
          .getResponse('GET', `company/company/search/plantillaxlsx/?name=${response}`)
          .then((resp: any) => {
            if (resp && resp.results && resp.results instanceof Array) {
              this.empresasListPlantillas = resp.results;
              let find = this.empresasListPlantillas.find(
                (x) => x?.name === response
              );
              this.setFormResultPlantillas(find);
            } else {
              this.empresasListPlantillas = [];
            }
          });
      });
  }
  ...
  setFormResult(find: any) {
    if (find) {
      ...
      const tempActorType = {
        ...
        proforest_actortype_code: this.empresaActive.actor_type.proforest_actortype_code,
  ...
  setFormResultReport(find: any) {
    if (find) {
      // console.log("setFormResultReport Set id:",find);
      this.empresaActiveReport = find;
      this.form.patchValue({
        id_empresa_reporta: this.empresaActiveReport.id,
        nit_reporta: this.empresaActiveReport.nit,
        //listCommodity: this.empresaActive.commodity,
        //listType_actor: this.empresaActive.actor_type,
      });
    } else {
  ...
  selectedName(e:any){
    ...
    /* console.log("8000 event:",e);
    console.log("8000 event:",e.srcElement.attributes ); */
    let selectedValue = e;
    this.setFilter2 = false;
    ...
    this.setFormResult(e);
    this.empresasList = [];
    let d:any = e;
    // console.log("8000 SELECTED d:",d  )
   /*  console.log("8000 selected d:",d  );
    console.log("8000 selected d2:",d.target.list.options[0].dataset  ); */
    
    //console.log("8000 selected event:",this.form.value);
    //let elem:any = document.getElementById("codes_report");
    //var text = elem?.options[e.selectedIndex].text;
    //console.log("8000 selected:",elem);
  }
  async delay(){
    return new Promise(resolve=>{
      setTimeout(function(){
  ...
  async setDatalistReportForm(e: any) {
    /* let count = 10000;
    console.log("setDatalistReport:",e.getAttribute("list"));
    var input:any = document.getElementById("codes_report");
    do{
      input = await document.getElementById("codes_report");
      console.log("input options:",input.options);
      if(input.options.length > 0){
        //e.options[e.selectedIndex].text;
        console.log("input selected:",input.selectedIndex);
        console.log("input id:",input.options[0].dataset);
      }else{
    ...
    let find = this.empresasListReport.find((x) => x?.name === e.value);
    this.setFormResultReport(find);
  }
  setDatalistReport(e: any): void {
    
    let find = this.empresasListReport.find((x) => x?.name === e.target.value);
    // console.log("8000 Set id:",find);
    this.setFormResultReport(find);
  }
  ...
  async downloadService() {
    ...
    let sendData = this.formPlantilla.value;
    let getToken: any;
    ...
    if (getToken) {
      return new Promise((resolve, reject) => {
        ...
        if (this.otroActorPlantilla) {
          urlTemp = `${this.path}traceability/api/xlsx/exportothers/?id_empresa_reporta=${sendData.id_empresa_plantilla}`;
        } else {
          urlTemp = `${this.path}traceability/api/xlsx/export/?id_empresa_reporta=${sendData.id_empresa_plantilla}`;
        }
        fetch(urlTemp, {
          method: 'GET',
          headers: headers,
          ...
          .then((data) => {
            this.empresaActivePlantillas.name
            let a = document.createElement('a');
            a.href = window.URL.createObjectURL(data);
            a.download = `trazabilidad_nit_${this.empresaActivePlantillas.nit}_${new Date()
              .getFullYear()
              .toString()}_${new Date().getMonth().toString()}`;
            a.click();
            resolve('success');
          })
          .catch((error) => {
  ...
  /*  Material pagination */
  displayedColumns: string[] = ['index', 'status', 'note', 'was_company_in_database'];
  ...
  uploadFile() {
    this.registerFile = false;
    //this.fileXlsx; empresaActivePlantillas
    ...
    if(!this.empresaActivePlantillas){
      this.validateFormP();
      return
    }
    if (this.otroActorPlantilla) {
      url = `traceability/xlsx/create/others/?id_empresa_reporta=${this.empresaActivePlantillas.id}`;
    } else {
      url = `traceability/xlsx/create/?id_empresa_reporta=${this.empresaActivePlantillas.id}`;
    }
  ...
  selectedReport(e:any){
    ...
    this.setFilter = false;
    this.form.patchValue({ 
      "name_report":this.selectedValue.name
    });
    this.setFormResultReport(e);
    this.empresasListReport = [];
    let d:any = e;
    // console.log("8000 SELECTED d:",d  )
   /*  console.log("8000 selected d:",d  );
