...
@Component({
  selector: 'app-matrix-select-multiple',
  templateUrl: './matrix-select-multiple.component.html',
  styleUrls: ['./matrix-select-multiple.component.scss']
})
export class MatrixSelectMultipleComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    const ifQuestion = JSON.parse(localStorage.getItem('tempQuestionMatrixSelectMultiple')!);
    ...
    if(this.form.valid){
      ...
      if(!this.update){
        ...
        .then( resp =>{        
          ...
          localStorage.removeItem('tempQuestionMatrixSelectMultiple');
          this.saveQuestion=true;
        }, error =>{
          this.loading=false;
  ...
  ngOnDestroy() {
      ...
      this.changeLang();
      localStorage.setItem('tempQuestionMatrixSelectMultiple', JSON.stringify(this.question))
    }
  }
