...
export class SelectOneComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    const ifQuestion = JSON.parse(localStorage.getItem('tempQuestionSelectOne')!);
  ...
  assingUpdate(){
    ...
    if(this.question.question_data.choices){
    ...
    if(this.form.valid){
      ...
      if(!this.update){
        ...
        .then( resp =>{        
          ...
          localStorage.removeItem('tempQuestionSelectOne');
          this.saveQuestion=true;
        }, error =>{
          this.loading=false;
  ...
  ngOnDestroy() {
    if(!this.saveQuestion){
      this.changeLang();
      localStorage.setItem('tempQuestionSelectOne', JSON.stringify(this.question))
    }
  }
