...
export class UploadFileComponent implements OnInit {
  ...
  constructor(
    ...
    private loadingService: EventService
  ) {
    this.question = {
      status: true,
      type: "US",
  ...
  ngOnInit(): void {
    ...
    const ifQuestion = JSON.parse(localStorage.getItem('tempQuestionUploadFile')!);
  ...
  async assingUpdate(){
    this.loadingService.loadEvent.emit(true);
    ...
    if(this.form.valid){
      ...
      if(!this.update){
        ...
        .then( resp =>{        
          ...
          localStorage.removeItem('tempQuestionUploadFile');
          this.saveQuestion=true;
        }, error =>{
          this.loading=false;
      ...
      if(resp.count>0){
        ...
        .then( (res:ListTopics) =>{
          ...
          if(this.update){
            ...
            this.loadingService.loadEvent.emit(false);
          }
        }, error =>{
          if(Array.isArray(error)){
            error.forEach((element:any) => {
  ...
  ngOnDestroy() {
    if(!this.saveQuestion){
      this.changeLang();
      localStorage.setItem('tempQuestionUploadFile', JSON.stringify(this.question))
    }
  }
