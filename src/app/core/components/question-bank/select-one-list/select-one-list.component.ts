...
import { Category, Subcategory, ListCategories, Topic, ListTopics, Question, QuestionSelectOne, TypeQuestion } from '../../../interfaces/form';
...
@Component({
  selector: 'app-select-one-list',
  templateUrl: './select-one-list.component.html',
  styleUrls: ['./select-one-list.component.scss']
})
export class SelectOneListComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    const ifQuestion = JSON.parse(localStorage.getItem('tempQuestionSelectOneList')!);
  ...
  createQuestion(){
    ...
    if(this.form.valid){
      this.loading=true;
      this.question.question_data['type'] = this.type;
      this.assingQuestion();
      this.changeLang();
      if(!this.update){
        ...
        .then( resp =>{        
          ...
          localStorage.removeItem('tempQuestionSelectOneList');
          this.saveQuestion=true;
        }, error =>{
          this.loading=false;
  ...
  ngOnDestroy() {
    if(!this.saveQuestion){
      this.changeLang();
      localStorage.setItem('tempQuestionSelectOneList', JSON.stringify(this.question))
    }
  }
